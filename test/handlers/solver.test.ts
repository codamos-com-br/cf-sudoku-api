...
test("POST /api/v1/solve returns 400", async () => {
	...
	const res = await handleRequest(
		new Request(`http://sudoku.codamos.com.br/api/v1/solve?input=${invalidLengthSudokuString}`, {
				method: "POST",
			}),
	);

	expect(res.status).toBe(400);
	expect(await res.text()).toBe('Invalid sudoku string: provide a 81-length digits stream.');

	const invalidCharactersSudokuString = 'a'.repeat(81);
	const res2 = await handleRequest(
		new Request(`http://sudoku.codamos.com.br/api/v1/solve?input=${invalidCharactersSudokuString}`, {
				method: "POST",
			}),
	);

	expect(res2.status).toBe(400);
	expect(await res2.text()).toBe('Invalid sudoku string: all characters must be a 0 to 9 digit.');
});
...
test("POST /api/v1/solve returns 500", async () => {
	...
	const res = await handleRequest(
		new Request(`http://sudoku.codamos.com.br/api/v1/solve?input=${impossibleSudoku}`, {
				method: "POST",
			}),
	);
